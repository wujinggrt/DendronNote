...
注意到，扩散策略每次新的 episode 总会往后抽一下，是否可以把每个 primitive 动作集成到 episode 中。DiT 预测噪声，或者是 latent，随后再加上观察。

观察应当是大的，长的 token，latent 应当是最靠近的。

#### 引导注意力

使用 mask 强行注入先验的效果很好。但是，第一帧用 VLM 获取 bbox，SAM2 获取 mask，随后每一帧都需要 Cuties 跟踪，workflow 比较复杂，且占用内存多。把语言跟随利用 mask 桥接起来。思考，利用 mask 的方式如此引导专家模型把注意力放到 mask 上，利用 msak 作为先验。

是否可以用更简便的方法？直接使用一个 CNN，EdgeNext-S 来识别和关注重点区域？利用 mask 制作数据集，利用多阶段对齐，参考 LLaVA，让小模型直接能够学习到语言与区域对应的能力。

### 对齐

LLaVA 使用 MLP 对齐了 LLM 与 ViT，是否可以训练一个 cross-embodiment 的模型，再使用 MLP 对齐 specific-embodiment 与 versatile model？

具有通用能力的大模型，与关心具体具身和执行的小模型，做好对齐十分关键。这样，扩展性即重新训练 MLP，重新插入扩散专家即可。
...
越能观察局部，就越牺牲了解全局；越关注全局就越忽略局部。这是矛盾的，需要 trade-off。
