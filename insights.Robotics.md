...
人会思考，不一定一直在执行动作。所以执行动作需要一个信号，这应该也体现在 VLA 的 VLM 中，规划好了，才给动作专家信号，进一步执行动作。似乎也像让 LLM 决定是否深度思考的工作。

VLM 和动作专家都要有空间理解能力。VLM 理解一系列动作帧、图像帧是靠近还是远离，动作专家收到指令，能执行靠近或远离。VLM 要有优秀的 spatial grounding 能力，比如 Qwen2.5VL 的标点和框，是必须的。
...
如何看待目前VLA的具身智能技术？ - 董子斌的回答 - 知乎
https://www.zhihu.com/question/1920708362489828723/answer/1922360752716617518

NORA：一个用于具身任务的小型开源通才视觉-语言-动作模型 - 黄浴的文章 - 知乎
https://zhuanlan.zhihu.com/p/1916713483724121443

VLA 的优势在于理解环境和人类意图，控制精度并非 VLM 部分擅长的，应当由动作专家完成。但是，当前 VLA 并没有充分发挥 VLM 的通识能力。经过大规模 Robotics 数据集的再训练，可能损失 VQA 能力。比如，pi0 的 PaliGemma 部分进行 prompting，得到胡言乱语。

同等数据集下的训练，VLA 消耗算力更大，小模型比如扩散模型，相同的 step 数表现更优。微调通常难以继承语言理解能力。

SmolVLA 仅使用 VLM 的前几层输出，效果直逼甚至超过 pi0，破坏了 VLM 的结构，但是仍然得到一个强的 VLA。是否可以推测，需要的是 Embedding。

基于扩散的 VLA，瓶颈在于 vision-language 理解，并非 action chunk 的生成。也就是动作生成比较优秀，但是任务理解不够突出。所以很多 VLA 工作把 VLM 当做 vision langauge embedding，并非需要通识能力。

我认为，应当用好 VLM，比如 Qwen2.5 VL 足够好的空间理解能力，标点、打框等足够，让动作专家足够。理解靠近这些点和框的概念即可。关键还是衔接动作专家的方式上，把 VLM 通识能力发挥好，用好其规划能力即可，不要因为动作专家更新梯度破坏了此项能力。

讨论 pi0，团队认为使用方式为微调，所以 zero shot 能力基本为 0。lerobot 建议 50 demos per task。但是 30K 的 steps，完全可以训练好扩散策略了。

VLM和LLM的能力毫无疑问是强大的，是一个通用决策模型需要的，而现在的VLA显然没有真的借到大模型的东风。

VLA应该朝哪个方向发展？放弃挂羊头卖狗肉换着法儿的Pick&Place，放弃臆想 yy 不存在的任务 setting 然后猛灌先验知识提点，放弃融合控制系统中的种种工程细节变成一个披着 VLA 皮的控制算法 pipeline 来降维打击 e2e。总之，放弃卷控制精度，回到对 VLA 期待的根本，AI 的任务不是要把一根细伞插进一支更细的伞桶，而是听到“帮我把伞放一下”的时候能自己观察到旁边的伞桶产生想把伞插进去的意图。正如其他答主所说的，VLA是好的，是面向未来的，但是暴力的不加思考的企图走捷径的VLA是注定失败的。

我认为，应当发挥好规划能力，VLM 有这种能力。需要探索动作专家理解空间能力，训练好反应能力。

### VLM 作为 Encoder 太过沉重的问题

重新设计视觉与语言。Qwen Embeddings 当做子任务的编码器。视觉如何设计？VLM 可以知道往哪儿看。

可以把耗时的任务，放到较大模型决策什么子任务时，输出 context，让动作专家根据 context 运行。本质还是异步的双模型概念。如何想出创新的部分？

使用标框的思路中，需要标注关注的区域，随后用 sam2 分割，提供 mask。比如，抓取物体场景应该分割出饮料瓶子和篮子，提供 context。

## 注入轨迹

我的思路，通过注入，让 VLM 自主选择是否需要复用缓存的轨迹，或者记录的轨迹，以解决 Corner Case。

记忆是重要的，缓存是重要的。在不同地区工作，收集不同数据，缓存了不同数据，造就了不同的解决问题能力。

我提出的是局部性原理，我的 insight 在于 cached 的场景。从而可以注入，甚至是 prefetch 轨迹。我认为执行任务时，有很多重复的动作可以复用，不同程度的复用。就像代码，组织良好了可以复用。那些组织良好的动作，应当就像放在商店的轨迹，可以复用。

移动到一个地方后，用动作专家执行精细的操作。**容忍失败和不精准，强调适配**，在下次可以找到更合理的程度。通过不断计算动作专家需要预测和调整的步数，适应性地找到最合适的轨迹执行程度。程度档次为五档：执行 0%, 25%, 50%, 75% 和 100%，在执行任务后，经过统计，根据家庭场景习惯，在多次执行后，可以调整到最佳的参数。

cached + 适配性的操作。

我需要分析大任务中，相似轨迹的数量。甚至可以分析数据集中，相似轨迹的数量。

### EiBT 的工作

他的见解在于记忆，从经验学习，不断地调整。认为是经验，沉淀的内容。

> RAG 使用轨迹来执行相关任务。

我使用相关轨迹，是为了复用，移动到对应的点，然后开始执行操作。

> 从过去经验学习，将经验用于新的场景。

使用技能树节点，表示动作的轨迹，人可以动态地添加动作序列相关节点，生成自主领域机器人的行为树，形成独有的解决能力。提供给 LLM 决策。

> 短期记忆模块能够分析执行成功的结果，动态反思和重新规划。

经验用于自我进化，我认为经验就是肌肉记忆，就是注入的指导，就像 MCP 一样提供的工具，给与参考答案，直接答题。

认为，从过去经验中，分析错误的地方，衡量哪儿没有做好，做出修改。

它使用自己生产的，我的工作提出，不仅仅使用自己生成的轨迹有效，人工注入、别的场景生成的，同样可以考虑进来。靠近动作和远离动作的轨迹，无非是相对机器人本体而言的动作，所以本质可以复用。

记录的轨迹有如下优点：
1. 可预测
2. 高效复用，节省推理的算力消耗，提高吞吐量。比如，抓取桌上的东西，可以先将手臂，ee 移动到桌面上方；收拾到篮子，可以将手臂移动到篮子上方。定义把“上方”为待命区域，staging area；把操作区域称为 operation area。复用应当发生在 staging area。

staging area 相关的操作关注靠近，关注头部相机，或全局相机。可以考虑关节角作为动作。比如 pi0，使用绝对关节角作为轨迹。

operation area 附近处理操作，关注腕部相机和本体状态，可以考虑 UMI，增量式的相对动作作为轨迹。

因为同时传入两个相机训练，引入不必要的环境，会增大模型的训练难度。

与 rule-based 的区别：
1. VLM 决策，规划是泛化的，rule-based 通常需要人类编程实现，而此工作更加关心注入的轨迹和轨迹的效果和影响
2. 

与记忆的区别：
1. 拓宽获取渠道，不仅仅从执行的记录获取，可以是人工注入，可以远程获取
2. 人为引导，面对新场景，模型没有遇见过的边缘场景，可以人类手动移动到 staging area，并将如此轨迹注入数据库或 BT 节点。

轨迹存在可复用的特点。轨迹数据存在局部性规律。
...
对于每个 episode，不应该只 shift 一个时间步，应当借鉴训练大模型的思路，不要有太多重复的 episode，或者是太多重复的时间步的动作，提高泛化能力。否则，模型可能会有过拟合的痛点，认为是在上一个点，所以表现得手会往回缩。
