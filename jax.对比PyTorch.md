...
变量 opt 保存梯度等状态，所以，Jax 写作 `opt = Optimizer(arg_opt); state = opt.init(params)`。

### 3. 模型计算

Jax 中，模型计算写作 `y = model.apply(params, x)`，其中 params 是模型参数。

### 4. 损失函数计算

因为损失函数不包含状态，所以与 PyTorch 一样，计算写作 `loss = loss_f(y, target)`，其中 y 是模型输出，target 是目标值。
...
此外，初始化函数 `__init__` 会修改为接受三个参数的函数。
...
`@flax.linen.compact` 是核心装饰器，简化模块的定义，自动处理参数管理。
...
但是，compact 装饰器与 setup 混用时，特别要注意所有名称唯一，避免冲突。

**参数共享问题**：

比如，前面的 x，3 次循环使用了相同的参数和模块。如果需要避免，还是用 setup 现实定义。
